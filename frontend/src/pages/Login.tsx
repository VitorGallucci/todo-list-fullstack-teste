...
export function Login() {
	...
	const handleLogin = async (e: React.FormEvent) => {
		...
		if (loading) return; // Proteção extra contra múltiplos envios
		...
		try {
			...
			if (error instanceof Error && 'response' in error) {
				...
				setErro(axiosError.response?.data?.erro || "Erro ao fazer login. Verifique suas credenciais.");
			} else {
					...
					<div>
						...
						<div className="relative">
							...
							<input
								...
								required
								disabled={loading}
								className="w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent outline-none transition-all disabled:bg-gray-50 disabled:text-gray-500"
